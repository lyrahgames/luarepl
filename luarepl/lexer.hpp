...
constexpr bool match(iterator& it, auto&& predicate) noexcept
  requires requires(codepoint c) {
    { invoke(forward<decltype(predicate)>(predicate), c) } -> same_as<bool>;
  }
{
  if (!invoke(forward<decltype(predicate)>(predicate), *it)) return false;
  ++it;
  return true;
}
...
using error_entry_base = variant<unfinished_long_comment>;
